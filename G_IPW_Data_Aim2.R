...
# Function for generate IPW analytical dataset
create_dat_tab = function(d, x, eoftype, covar, trt){
  ...
  dat2$startA = as.Date(dat2$startA)
  dat2$endA = d[,trt_end]
  dat2$endA = as.Date(dat2$endA)
...
# Read the merged aim 2 datasets (sent by KP Secure Transfer)
phs.aim2.data = read.csv("phs_aim2_data.csv")

# Create eofdate variable
...
covar_chemo = c("ajcc_stage", "diab_bl", "htn_bl", "dyslipid_bl", "bmicat1", "smok", "charlson", "edu_cat", "income_cat",
                "agegrp", "race", "horm_yn","rad_tx_yn", "menop") # covariates

# Indicator for excluding prevalent cvd 
